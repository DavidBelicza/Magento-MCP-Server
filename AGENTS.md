...
- `src/api/stream/status.ts` (route `GET /api/stream/status`) + `src/modules/stream/streamer.ts` + `src/modules/stream/status-events.ts`: a push channel for status. Producers (the worker on job `active`/`progress`/`completed`/`failed` via `src/modules/stream/forward-index-events.ts`, and the enqueue routes) publish a tiny typed flag to one Redis pub/sub channel `magentic:status-events` (`{ "type": "index" }`); the backend keeps a **single** subscriber that, per type, debounces, re-reads the snapshot, and broadcasts it as a Server-Sent Event over `GET /api/stream/status`. Constant 2 extra Redis connections regardless of queue count (1 publisher in the worker, 1 subscriber in the backend). The dispatcher is a `type → snapshot-handler` registry, so a future producer (e.g. a long-search job whose data lives outside Redis) just publishes its own `{ "type": ... }` flag and registers a handler. Final-state correctness: the worker publishes **after** releasing its lock, so the broadcast never reports a stale `locked`. The frontend (`StatusContext`) consumes the stream via a `fetch`-reader (so the bearer header applies — native `EventSource` cannot set it), replaces status on each `event: index`, auto-reconnects with backoff, and keeps a slow 59s safety poll for agent freshness + resync.
