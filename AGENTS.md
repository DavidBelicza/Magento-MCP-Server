...
- `magentic_embedder`: bundled embedding model — a `llama.cpp` server built multi-stage (only `/app` from `ghcr.io/ggml-org/llama.cpp:server` copied onto `ubuntu:24.04`, ~470 MB/arch) with the `embeddinggemma-300m` GGUF (768 dims) baked into the image, exposing an OpenAI-format `/v1/embeddings` on port 8080. **Always enabled** (not profile-gated) so semantic search works out of the box with no external LM Studio. The backend/worker pick which embedder to call via `EMBEDDER_TYPE` (`local` default → this service; `remote` → an external server, defaults to LM Studio); the two providers are configured by symmetric `LOCAL_EMBEDDER_*` and `REMOTE_EMBEDDER_*` env sets. The image is multi-arch (the GGUF file is arch-independent; only the llama.cpp binary differs) and built by the same release matrix as the other images. Switching `EMBEDDER_TYPE` requires a vector reset-and-reindex (stored and query vectors must come from the same model).
...
- `src/api/config/`: `GET /api/config` and `PUT /api/config` for user-editable runtime settings, plus `GET /api/graph/stats` (Neo4j node/edge counts) under `src/api/graph/stats.ts`. Settings are file-backed by `src/modules/app-config.ts` (JSON at `MAGENTIC_CONFIG_PATH`, default `/app/data/config.json`, bind-mounted from `./data` so it survives rebuilds) — loaded into memory at backend startup and rewritten only on UI save; manual file edits need a backend restart. Settings: `phpVersion` (parser target, threaded into the index-source job and on to the PHP analyzer's `ParserFactory`), `projectRoot` (where `composer.lock` is read, joined onto the mount in `getComposerRoot`), `sourceSubpaths` (a list of directories scanned for PHP, each in its own analyzer pass; empty = whole mount), and the **active embedder** — `embedderType` (`local` | `remote`) plus the editable `remoteEmbedderUrl`/`remoteEmbedderModel`/`remoteEmbedderBearerToken` (the local embedder stays env-only and read-only; `GET /api/config` returns it as `localEmbedder` for display). `readEmbeddingConfig()` resolves the active `EmbeddingConfig` from these settings (remote) or env (local); the backend resolves it per search request and snapshots it into the vector job at enqueue, so the worker uses the job's config (the worker has no `config.json` mount). The worker never reads the file — the backend snapshots these into job data at enqueue time. The single index-source job loops the directories (worker concurrency is 1, so per-directory queue jobs would not parallelize anyway) and reports per-directory progress.
- `src/config.ts`: environment-backed config, including `GRAPH_BATCH_SIZE` (source ingestion batch and transaction size, default 5000), `POSTGRES_VECTOR_URL` (the `magentic_pgvector` connection), and the embedder settings: `EMBEDDER_TYPE` (`local` default | `remote`) selecting between two symmetric sets — `LOCAL_EMBEDDER_URL`/`LOCAL_EMBEDDER_MODEL`/`LOCAL_EMBEDDER_BEARER_TOKEN` (default the in-cluster `magentic_embedder`) and `REMOTE_EMBEDDER_URL`/`REMOTE_EMBEDDER_MODEL`/`REMOTE_EMBEDDER_BEARER_TOKEN` (default LM Studio at `host.docker.internal:1234/v1/embeddings`). `readEmbeddingConfig()` resolves the active set into an `EmbeddingConfig` (full OpenAI-format endpoint, model, nullable bearer token).
