...
- `src/queue/index-vector.ts` + `src/worker/index-vector-worker.ts`: the `index-vector` BullMQ pipeline (`operation: "index" | "reset"`), wired into the single `magentic_worker`. Triggered by `POST /api/vector/index` (reindex) and `DELETE /api/vector/index` (reset), guarded by `magentic:vector-index:lock` (independent of the graph's `magentic:graph-index:lock`). `POST /api/vector/search` embeds a query and returns the top matches.
