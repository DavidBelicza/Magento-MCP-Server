...
Dev Compose overrides are in `docker-compose.dev.yml`. Backend/worker/site source mounts allow most source edits without rebuilding. Rebuild only when package metadata or Dockerfiles change.
